...
static void event_handler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data)
{
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        ...
        ESP_LOGI(TAG, "Disconnection. Trying to connect...");
        esp_wifi_connect();
...
void wifi_init_station(
    ...
    ESP_ERROR_CHECK(
        esp_event_handler_instance_register(IP_EVENT, IP_EVENT_STA_GOT_IP, connected_callback, NULL, &instance_got_ip)
    );
    ESP_ERROR_CHECK(
        esp_event_handler_instance_register(WIFI_EVENT, WIFI_EVENT_STA_DISCONNECTED, disconnected_callback, NULL, &instance_disconnected)
    );
